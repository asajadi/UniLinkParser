...
struct Parse_Options_s {
  ...
  int unify_features;			/* if true, the unificaton process is done
								on the linkage, A.Sajadi*/
  int display_features;			/* if true, all features is printed, A.Sajadi*/
};
...
/*A.S*/
//Structures for morphological analysis
#define MDWL 2*MAX_WORD //Max length set to 20
#define USE_INTERNAL_MEM 0 
#define USE_EXTERNAL_MEM 0 
#define maxdecom 50
#define maxcat 50

typedef  struct Decomposed_word_s Decomposed_word;
typedef  struct Morpho_structures_s * Morpho_structures;
typedef  struct Morpheme_s Morpheme;
struct Morpheme_s{
	char* word;
		Feature_list *f_list;
	Morpheme *next;
};
struct Decomposed_word_s{
	   char word[MDWL];
	   Decomposed_word * next;
	   Decomposed_word * prev;
};

struct Morpho_structures_s{
	Dictionary dict;
	Parse_Options opts;
	Morpheme *morpheme;
};
...
struct Dictionary_s {
    ...
    int             already_got_it;
    int             line_number;
	Feature_list *		shared_features;  /*A.S*/
	char *			feature_template_file_name;  /*A.S*/
	char *			feature_equ_file_name;  /*A.S*/
	char *			current_file_name;/*A.S*/
	Feature_template_set	*	feature_template_set;/*A.S*/
	Feature_conn_set		*	feature_conn_set;/*A.S*/
	Feature_label_node *	fln;/*A.S*/
    char    *		feature_token;/*A.S*/
	Morpho_structures morpho_structures;/*A.S morpho*/

	//char *			sub_sensitive_links_file;/*A.S*/
	//String_list * sub_sensitive_links_list;/*A.S*/
...
struct Parse_info_struct {
    ...
    Feature *      feature_array[MAX_SENTENCE];/*A.Sajadi*/
    int            N_links;
...
struct Sentence_s {
    ...
    int * unification_info;    /* unified linkage linkages (sorted) 
							   Added by armin Sajadi*/
...
struct Linkage_s {
    ...
    Sentence        sent;
    Parse_Options   opts;
    Feature *      *feature_array;/*A.Sajadi*/
