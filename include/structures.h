...
#define MAX_TOKEN_LENGTH 100          /* maximum number of chars in a token */
...
#define NO_WORD 255
/***************************************
Some structures to implement features and unification
by A.S
****************************************/
#define MAX_FEATURE_LABEL 10 
#define MAX_FEATURE_ATT 40 
#define MAX_DICT_MORPH 3 
#define USE_SOURCE_STRING_SET 0
#define ROOT_FEATURE "root"
#define HEAD_WORD "head"
typedef enum{USE_SHARED, IS_SHARED, SHST_NONE} Share_stat;
typedef enum{AGR_PATH, AGR_FEATURE, AGR_NOT_INITED} Agr_stat;

typedef struct Feature_struct Feature;
typedef struct Feature_list_struct Feature_list;
struct Feature_struct {
	char * attribute;
	Share_stat share_stat;
	Feature * child;
	Feature * next;
	Feature_list *parent;
};
...
typedef struct Feature_label_node_struct Feature_label_node;
struct Feature_label_node_struct{
	char label[MAX_FEATURE_LABEL];
	Feature * f;
	Feature_label_node * next;
};

typedef struct Feature_template_struct Feature_template;
struct Feature_template_struct {
	char * name;
	Feature  * f;
};


typedef struct Feature_template_set_struct Feature_template_set;
struct Feature_template_set_struct{
    int size;       /* the current size of the table */
    int count;      /* number of things currently in the table */
	Feature_template ** table;  /* the table itself */
};
...
typedef union Feature_agreement_union Feature_agreement;
union Feature_agreement_union{
	String_list *path;
	Feature *f;
};
typedef struct Feature_equ_struct Feature_equ;
struct Feature_equ_struct {
	Agr_stat agr1_stat; 
	Agr_stat agr2_stat; 
	Feature_agreement  agr1;
	Feature_agreement  agr2;
	Feature_equ * next;
};

typedef struct Feature_connector_struct Feature_connector;
struct Feature_connector_struct {
	char * conn_name;
	Feature_equ * equ;
};

typedef struct Feature_conn_set_struct Feature_conn_set;
struct Feature_conn_set_struct{
    int size;       /* the current size of the table */
    int count;      /* number of things currently in the table */
	Feature_connector ** table;  /* the table itself */
};

typedef struct Feature_copy_table_struct Feature_copy_table;
struct Feature_copy_table_struct{
	Feature *old_f;
	Feature *new_f;
	Feature_copy_table *next;
};
