...
/*
   ...
   types.  foo is the driver link, bar and baz are called the criterion
   ...
   While the previous pass caused a count to go to 0 do:
      For each connector C do
         For each rule R do
            if C is a driver for R and the criterion links
	...
	for (p = ls->hash_table[hashval]; p!=NULL; p=p->next) {
		...
		*s = '#';
	    ...
	    /* now if bad==0 this criterion link does the job
	       to satisfy the needs of the driver link */
	...
	for (w = 0; w < sent->length; w++) {
		...
		for (dir=0; dir < 2; dir++) {
			...
			for (i=0; i<knowledge->n_contains_one_rules; i++) {
			    ...
			    /*
			    printf("pp_prune: driver ok.  selector = %s  c->string = %s\n", selector, c->string);
			    */

			    /* We know c matches the driver link of the rule. */
