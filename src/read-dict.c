...
void dict_error(Dictionary dict, char * s) { /*static deleted by Arimn Sajadi*/
    int i;
...
Exp * restricted_expression(Dictionary dict, int and_ok, int or_ok) {
    ...
    /*
	if (is_equal(dict, '[')){
		if (!(nl->Feature_list=read_connector_feature(dict,nl->Feature_list)))
			return NULL;
	if (!advance(dict)) {
			return NULL;
		}
...
/* Add these words to the dictionary                                   */
    Exp *n;
    int i;
	int word_file_feature;
    Dict_node  *dn_new, *dnx, *dn = NULL;

    for (; !is_equal(dict, ':') ; advance(dict)) {
		/*A.S*/
	  dict->feature_fp = dict->fp;/*A.S*/
	  dict->current_file_name = dict->name;
	  if ((dn!=NULL) && (is_equal(dict, '['))){
		if (word_file_feature==FALSE){
			if (!(dn->f=feature_read_driver(dict))){
					return FALSE;
				}
		}
		else
		{
			dict_error(dict, "Feature definition  not allowed here");
			return 0;
		}
		if(!is_equal(dict, ']')){
			return FALSE;
	...
	if (string_is_equal(dict->token, '|')) {
		...
		if (!read_general_word_file_features(dict,dn)){
				return FALSE;
		}
		if(!string_is_equal(dict->token, ']')){
			return FALSE;
