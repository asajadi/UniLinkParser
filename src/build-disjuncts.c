...
Disjunct * build_disjunct(Clause * cl, char * string, int cost_cutoff, Feature *feature) {/* Edited by A.S*/
...
Disjunct * build_disjuncts_for_X_node(X_node * x, int cost_cutoff) {
	...
	dis = build_disjunct(c, x->string, cost_cutoff, x->feature); /* Edited by A.S*/
    free_clause_list(c);
    return dis;
}
...
/*                 print_clause_list(c);        */
	dis = build_disjunct(c, dn->string, NOCUTOFF, dn->f);
    free_clause_list(c);
    return dis;
}
...
*/
	...
	x->string = dn->string;
	x->feature = feature_copy_driver(USE_SOURCE_STRING_SET, dn->f); /*A.S*/
	dn = dn->right;
    }
    return x;
}
