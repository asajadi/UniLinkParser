...
Parse_Options parse_options_create() {
  /* create and initialize a Parse_Options object */
	...
	po->display_features = TRUE;//A.S
	po->unify_features = TRUE;//A.S
...
/*A.Sajadi*/
void parse_options_set_display_features(Parse_Options opts, int dummy) {
	opts->display_features = dummy;
}
int parse_options_get_display_features(Parse_Options opts) {
	return opts->display_features;
}
void parse_options_set_unify_features(Parse_Options opts, int dummy) {
	opts->unify_features = dummy;
}
int parse_options_get_unify_features(Parse_Options opts) {
	return opts->unify_features;
}
/*A.Sajadi*/
 ...
 */
static Dictionary internal_dictionary_create(char * dict_name, char * pp_name, char * cons_name, char * affix_name, char * path, char *morpho_dict_name) { /*Edited by A.S morpho*/
    Dictionary dict;
    static int rand_table_inited=FALSE;
    Dict_node *dict_node;
    char * dictionary_path_name;
	char temp_file_name[MAX_PATH_NAME];
	...
	dict->feature_template_set = feature_template_set_create(); /*A.S*/
	dict->shared_features = NULL; /*A.S*/
	dict->feature_conn_set = feature_conn_set_create(); /*A.S*/
	dict->feature_token = NULL; /*A.S*/
	dict->fln = NULL; /*A.S*/
	dict->feature_template_file_name = NULL;/*A.S*/
	dict->current_file_name = NULL;/*A.S*/
	dict->morpho_structures=NULL;/*A.S*/
	//dict->sub_sensitive_links_list = NULL;/*A.S*/
    /*  *DS*  remove this
    if (pp_name != NULL) {
    ...
    */
	...
	/*A.S*/
	make_file_name(temp_file_name, dict->name, "features");
	dict->feature_template_file_name = string_set_add(temp_file_name, dict->string_set);
	if(feature_templates_read(dict, dictionary_path_name)==FALSE){
		return 0;
	}
	make_file_name(temp_file_name, dict->name, "uni");
	dict->feature_equ_file_name = string_set_add(temp_file_name, dict->string_set);
	if(feature_conn_read(dict, dictionary_path_name)==FALSE){
		return 0;
	}

	//make_file_name(temp_file_name, dict->name, "links");
	//dict->sub_sensitive_links_file = string_set_add(temp_file_name, dict->string_set);
	//read_links_list(dict, dictionary_path_name);

	if(morpho_dict_name != NULL){
		dict->morpho_structures=(Morpho_structures)xalloc(sizeof(struct Morpho_structures_s));
		dict->morpho_structures->dict=
			internal_dictionary_create(morpho_dict_name, NULL, NULL, NULL, dict_name, NULL);
		if (dict->morpho_structures->dict == NULL) {
			fprintf(stderr, "%s\n", lperrmsg);
			exit(-1);
		}
		dict->morpho_structures->opts=parse_options_create();
		dict->morpho_structures->morpheme=NULL;
    ...
    dict->affix_table = NULL;
    if (affix_name != NULL) {
	dict->affix_table = internal_dictionary_create(affix_name, NULL, NULL, NULL, dict_name, NULL); /*Edited by A.S morpho*/
...
Dictionary dictionary_create(char * dict_name, char * pp_name, char * cons_name, char * affix_name, char* morpho_dict_name) { /*Edited by A.S morpho*/
	return internal_dictionary_create(dict_name, pp_name, cons_name, affix_name, NULL, morpho_dict_name);/*Edited by A.S morpho*/
}
...
int dictionary_delete(Dictionary dict) {
	...
	if(dict->morpho_structures!=NULL){
		delete_morpho_structures(dict->morpho_structures);
	}
    ...
    string_set_delete(dict->string_set);
	feature_template_set_delete(dict->feature_template_set);  /*A.S*/
	feature_conn_set_delete(dict->feature_conn_set);;  /*A.S*/
    free_dictionary(dict);
	feature_list_free(dict->shared_features);/*A.S*/
    xfree(dict, sizeof(struct Dictionary_s));
...
void post_process_linkages(Sentence sent, Parse_Options opts) {
    ...
    /*A.S*/
	unify_sentence(sent, opts);
	/*A.S*/
	unification_info = sent->unification_info;
	...
	for (in=0; in<N_linkages_alloced; in++) {
		indices[in] = unification_info[in]; /*Edited by A.S*/
	}
    }
    else {
	...
	for (in=0; in<N_linkages_alloced; in++) {
	    ...
	    indices[in] = unification_info[block_bottom + (my_random() % (block_top-block_bottom))]; /*Edited by A.S*/
	}
	my_random_finalize();
    }
...
void free_post_processing(Sentence sent) {
	...
	xfree((char *) sent->unification_info, sent->num_linkages_found*sizeof(int));
...
Linkage linkage_create(int k, Sentence sent, Parse_Options opts) {
	...
	linkage->feature_array = sent->parse_info->feature_array;/*A.Sajadi*/
	if (opts->unify_features)
		unify_linkage(sent);/*A.Sajadi*/
    compute_chosen_words(sent, linkage);
    if (set_has_fat_down(sent)) {
//	extract_fat_linkage(sent, opts, linkage);//A.S
	extract_thin_linkage(sent, opts, linkage);//A.S
    }
    else {
