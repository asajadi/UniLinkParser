...
void compute_chosen_words(Sentence sent, Linkage linkage) {
    ...
    Feature * word_features[MAX_SENTENCE]; /*A.S*/
    Parse_Options opts = linkage->opts;
	...
	word_features[i] = pi->chosen_disjuncts[i]->feature;
    }
...
char * linkage_print_diagram(Linkage linkage) {
    ...
    *s = '\0';
	...
	//Added by A.S
	if(opts->display_features){
        t = print_features(linkage); 		
		gr_string = strappend(gr_string, t);
		string_delete(t);
	}
	//Added by A.S
    return gr_string; 
